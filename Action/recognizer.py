...
#
#            # 트랙의 결과를 기록합니다. bounding boxes와 ID 포함
#             trk_result = []
...
#                 # id = int(d[4])
#                 try:
#                    # xcenter는 한 프레임의 이미지 속 모든 휴먼의 1번 관절점(neck)의 x좌표값입니다.
#                    # track_box와 human의 xcenter 사이의 거리를 계산하여 ID를 일치시킵니다
#                     tmp = np.array([abs(i - (xmax + xmin) / 2.) for i in xcenter])
#                     j = np.argmin(tmp)
#                 except:
#                     # 현재 프레임에 휴먼이 없으면 기본 j = 0 (올바르지 않음)
#                     j = 0
#
#                 # 동작 구분을 진행합니다
...
def framewise_recognize(pose, pretrained_model):
    ...
    if bboxes:
        ...
        for d in trk_result:
            ...
            # id = int(d[4])
            try:
                # xcenter 는 한 프레임의 이미지 속 모든 휴먼의 1번 관절점(neck)의 x좌표값
                # 트랙킹 박스와 휴먼의 xcenter 사이의 거리를 계산하여 ID를 일치 시킴
                tmp = np.array([abs(i - (xmax + xmin) / 2.) for i in xcenter])
                j = np.argmin(tmp)
            except:
                # 현재 프레임에 휴먼이 없으면 기본 j=0 (올바르지 않음)
