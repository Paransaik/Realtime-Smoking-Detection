...
from collections import namedtuple #collections 모듈 아래있음 namedtuple
from .coco_format import CocoPairs, CocoPairsNetwork #coco_format 파일에서
from .human import Human #pose파일에 있는 human 안에 Human클래스 불러옴
from scipy.ndimage import maximum_filter, gaussian_filter #물체 분포?


heatmap_supress = False #히트맵 압박?
heatmap_gaussian = True #히트맵 스위트
adaptive_threshold = False #적응형 태도적

NMS_Threshold = 0.15  #비-최대 억제 예측, 검출해낸 박스중에 가장 정확한 박스 선택 하는 비율?
...
#nms 함수 모델이 예측한 bbox중 겹치는 부분이 많은 것 제거
def non_max_suppression(plain, window_size=3, threshold=NMS_Threshold): #창크기3
    under_threshold_indices = plain < threshold #미달
